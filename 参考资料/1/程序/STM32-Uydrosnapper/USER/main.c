...
//准备下沉
		Target_depth=70;Target_pitch=0;
		yaw_Reset();
		while(yaw!=0){};
		isposeloopclosed=1;
		isyawloopclosed=1;
...
void Board_Init(void)
{
	 ...
	 Raspberry_Usart2_Init(115200);//串口初始化为115200,接收Raspberry pi数据
...
//	TIM2_LEDIndicator_Init(1999,35999);//1s一次
	//这些初始化如果放在电调的前面，解锁就会失败
	 Lora_Usart3_Init(9600);	//串口初始化为9600，接收无线串口指令,发送中断开着，虽然不知道有啥用
	 IMU_Usart1_Init(9600);	 //串口初始化为9600,接收JY901数据
	 IIC_Init();HP206C_Init();//压力传感器初始化必须在水面上开机，以确定大气压
	 PID_pos_init();	 
	 TIM4_LocSettlement_Init(3000,7199);

//这个定时器中断关掉的话，压力数据就不会更新了
	 TIM6_PosPID_Init(2499,7199);//10Khz的计数频率，计数到2500为250ms,姿态深度PID计算	
	 TIM7_LinePID_Init(3499,7199);//10Khz的计数频率，循环周期为0.35s,巡线角度距离PID计算
}
