...
void Setpwm_UD(void)
{
	...
	forward_pwm =  -(forward+43);//91;
	behind_pwm =  behind+44;//79;

	forward_pwm=constrain(forward_pwm,-200,-43);//确保始终在转
	behind_pwm=constrain(behind_pwm,44,200);
...
void Setpwm_LR(void)
{
	...
	left_pwm =  left+58*(left>0)-43*(left<0);//
	right_pwm =  right+51*(right>0)-50*(right<0);//

	left_pwm=constrain(left_pwm,-200,200);//arr设为20000，水下也许一点点推力就够了
	right_pwm=constrain(right_pwm,-200,200);
...
{
	TIM_SetCompare3(TIM1,1500-constrain(left,-200,200));
	TIM_SetCompare3(TIM3,1500+constrain(right,-200,200));
	TIM_SetCompare2(TIM3,1500+constrain(forward,-200,200));
	TIM_SetCompare1(TIM1,1500+constrain(behind,-200,200));
}
