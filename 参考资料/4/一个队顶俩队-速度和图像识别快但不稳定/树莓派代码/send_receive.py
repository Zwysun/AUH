...
def detectyellowball(stack, flag, lock):
    ...
    while True:
        ...
        low_range0 = np.array([13, 124, 70])
        high_range0 = np.array([55, 255, 255])
    ...
    while True:  #循环重新启动串口
        ...
        if data==0:
            ...
            while len(q2) == 0:
                pass
            frame2 = q2[-1]
            cv2.imwrite('./00.jpg',frame2)
            alphabet_class=detect_alpha(frame2)
            alphabet_data="%x%02x%02x%02x%04x%02x%02x%02x"%(head,length_alphabet,count,ID_alphabet,alphabet_class,CRC,tile,tile)
            print(alphabet_data)
            ...
            print('catch green circle')
            while len(q1) == 0:
                pass
            frame = q1[-1]
            cv2.imwrite('./capture_green/'+str(green_circle_num)+'.jpg',frame)
            while len(q1) == 0:
                pass
            frame = q1[-1]
            cv2.imwrite('./capture_green/d'+str(green_circle_num)+'.jpg',frame)
            green_circle_num = green_circle_num+1
        else:
            depth=data
            print(depth)
