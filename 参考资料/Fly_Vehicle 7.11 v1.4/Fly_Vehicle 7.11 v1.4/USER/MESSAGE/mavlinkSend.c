...
void MavlinkSendHeartbeat(uint8_t* sendFlag)
{
    ...
    heartbeat.autopilot     = MAV_AUTOPILOT_ARDUPILOTMEGA;    //设置飞控类型为PX4，以便能使用QGroudControl地面站
...
//    status.load = Get_OSCPUusage() * 10;
    status.voltage_battery = 15500;
    status.current_battery =1500;
...
void MavlinkSendScaledImu(uint8_t* sendFlag)
{
    ...
    //消息负载赋值
    scaled_imu.time_boot_ms = GET_TICK_COUNT();
    scaled_imu.xacc         = 30;//AccGetData().x * 1000+(rand()%11);
    scaled_imu.yacc         = 40;//AccGetData().y * 1000;
    scaled_imu.zacc         = 50;//AccGetData().z * 1000;
    scaled_imu.xgyro        = 60;//Radians(GyroGetData().x) * 1000;
    scaled_imu.ygyro        = 70;//Radians(GyroGetData().y) * 1000;
    scaled_imu.zgyro        = 80;// Radians(GyroGetData().z) * 1000;
    scaled_imu.xmag         = 90;//MagGetData().x * 1000;
    scaled_imu.ymag         = 100;//MagGetData().y * 1000;
    scaled_imu.zmag         = 200;//MagGetData().z * 1000;
...
void MavlinkSendAttitude(uint8_t* sendFlag)
{
    ...
    //消息负载赋值
    attitude.time_boot_ms = GET_TICK_COUNT();//GetSysTimeMs();
    attitude.roll         = 11;//Radians(GetCopterAngle().x);
    attitude.pitch        = 12;//Radians(GetCopterAngle().y);
    attitude.yaw          = 13;//Radians(GetCopterAngle().z);
    attitude.rollspeed    = 14;//Radians(GyroGetData().x);
    attitude.pitchspeed   = 15;//Radians(GyroGetData().y);
    attitude.yawspeed     = 16;//Radians(GyroGetData().z);
...
void MavlinkSendLocalPositionNed(uint8_t* sendFlag)
{
    ...
    //消息负载赋值
    position.time_boot_ms = GET_TICK_COUNT();//GetSysTimeMs();
    position.x            = 1000;//GetCopterPosition().x;
    position.y            = 2000;//GetCopterPosition().y;
    position.z            = 3000;//-GetCopterPosition().z;
    Vector3f_t velEf;
    //TransVelToEarthFrame(GetCopterVelocity(), &velEf, GetCopterAngle().z);
    position.vx           = 5000;//velEf.x;
    position.vy           = 6000;//velEf.y;
    position.vz           = 7000;//-velEf.z;
...
void MavlinkSendRcChannels(uint8_t* sendFlag)
{
    ...
    //消息负载赋值
    rc.time_boot_ms = GET_TICK_COUNT();//GetSysTimeMs();
    rc.chancount    = 8;
    rc.chan1_raw    = 1;//GetRcData().roll;
    rc.chan2_raw    = 2;//GetRcData().pitch;
    rc.chan3_raw    = 3;//GetRcData().throttle;
    rc.chan4_raw    = 4;//GetRcData().yaw;
    rc.chan5_raw    = 5;//GetRcData().aux1;
    rc.chan6_raw    = 6;//GetRcData().aux2;
    rc.chan7_raw    = 7;//GetRcData().aux3;
    rc.chan8_raw    = 8;//GetRcData().aux4;
    rc.chan9_raw    = 9;//GetRcData().aux5;
    rc.chan10_raw   = 10;//GetRcData().aux6;
    rc.chan11_raw   = 11;//GetRcData().aux7;
    rc.chan12_raw   = 12;//GetRcData().aux8;
    rc.chan13_raw   = 13;//GetRcData().aux9;
    rc.chan14_raw   = 14;//GetRcData().aux10;
    rc.chan15_raw   = 15;//GetRcData().aux11;
    rc.chan16_raw   = 16;//GetRcData().aux12;
